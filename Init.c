...
//	printf("ring#=%d\tRAD_MED=%f\n",i,dust_budget[i].rad_med);
	...
	for(j=0;j<peb_size_num;j++){
		...
		peb_map[i].vr_med_s[j]=vr_estimate(peb_map[i].rad,peb_map[i].size_med[j],pp_vr_tau1);
                //tau=pp_vr_tau1[1];
                //peb_map[i].vt_med_s[j]=0.5*tau*peb_map[i].vr_mid[j];
	//	if(i==1) printf("SIZE=%fcm\n",peb_map[i].size[j]);
        	peb_map[i].vr_med_r[j]=vr_estimate(peb_map[i].rad_med,peb_map[i].size[j],pp_vr_tau1);
                tau=pp_vr_tau1[1];
