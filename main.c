	...
	char *argv[];
{
   ...
   // group();
	...
	Init2();
	check_disk(1.0);
	drift_vr_test(1.0,22.0,pp_vr_tau2);
	...
	a_pb1=i*0.1;
	//a_pb1=9.0;
        vr1=vr_estimate(1.0,a_pb1,pp_vr_tau0);
        tau1=pp_vr_tau0[1];
        vr2=drift_vr(1.0,a_pb1,pp_vr_tau2);
        tau2=pp_vr_tau2[1];
	re1=2.0*a_pb1*sqrt(vr1*vr1+0.25*tau1*tau1*vr1*vr1)/viscosity(1.0);
	printf("SIZE=%f\n",a_pb1);
        fprintf(fp,"%f %f\t est= %f\t%f\t%2.10f\t%f\t%2.10f\n",2.25*mean_path(1.0),a_pb1,vr1,2.0*a_pb1*sqrt(vr1*vr1+0.25*tau1*tau1*vr1*vr1)/viscosity(1.0),\
                vr2,2.0*a_pb1*sqrt(vr2*vr2+0.25*tau2*tau2*vr2*vr2)/viscosity(1.0),v_r1_test(1.0,a_pb1));
