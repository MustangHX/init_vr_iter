...
{
	double drag_coeff,drag_force,Re,tau_fric,tau,vr,output;
	int judge=0;
	Re=2.0*a_pb*(v_K(r)-vt_gas(r))/viscosity(r);
	if(a_pb<2.25*mean_path(r)){
...
void vr_iter(double r, double a_peb, double vr1, double vt1, double *vr2, double *vt2){
		double drag_coeff,drag_force,Re,tau_fric,power;
...
double drift_vr(double r,double a_peb, double *p_vr_tau)
{
	...
	count=0;
        do{
                vr1=0.1*vr2+0.9*vr1;
                vt1=0.1*vt2+0.9*vt1;
                vr_iter(r,a_peb,vr1,vt1,&vr2,&vt2);
                count++;
